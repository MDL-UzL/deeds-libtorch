...
from __init__ import SRC_DIR, BUILD_DIR, BUILD_JIT_DIR, TEST_DATA_DIR, CPP_APPLY_BCV_MODULE, test_equal_tensors
global u1_,v1_,w1_
class TestApplyBCV(unittest.TestCase):
    ...
    def test_main_case_five(self):
        ...
        TEST_BASE_DIR = Path(TEST_DATA_DIR, "./case_5").resolve()
        OUTPUT_DIR = TEST_BASE_DIR.joinpath("test_output")
        OUTPUT_DIR.mkdir(exist_ok=True, parents=True)
        MOVING_INPUT_FILE = Path(TEST_BASE_DIR, "label_moving_cubic.nii.gz").resolve()
        FLOW_INPUT_FILE = Path(TEST_BASE_DIR, "./deeds_bcv_output/case_5").resolve() # Resolves to "zero_flow_displacements.dat"
        AFFINE_MATRIX_FILE = Path(TEST_BASE_DIR, "./linear_bcv_output/linear_bcv_affine_matrix.txt").resolve()
